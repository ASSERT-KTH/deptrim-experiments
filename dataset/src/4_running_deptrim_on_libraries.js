...
const jdblDatasetPath = path.resolve(__dirname, 'dataset/dependencies.json');
...
function deptrim(projectNameAndRelease, projectName) {
  shell.cd(librariesPath + '/' + projectNameAndRelease + '/' + projectName);
  shell.exec(
    'mvn se.kth.castor:deptrim-maven-plugin:0.0.1:deptrim -DcreatePomTrimmed=true -DignoreScopes=test,provided,system,import,runtime  > deptrim_execution_log.txt'
  );
}
